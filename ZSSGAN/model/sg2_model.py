...
def make_kernel(k, dtype=torch.float32):
...
class Blur(nn.Module):
    def __init__(self, kernel, pad, upsample_factor=1):
        ...
        # kernel = make_kernel(kernel, dtype=torch.float16)
        kernel = make_kernel(kernel)
...
class EqualLinear(nn.Module):
    def __init__(
        ...
        if bias:
            # self.bias = nn.Parameter(torch.zeros(out_dim, dtype=torch.float16).fill_(bias_init))
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if not input_is_s_code:
            return self.forward_with_w(styles, return_latents, inject_index, truncation, truncation_latent, input_is_latent, noise, randomize_noise, return_feats=return_feats)
    ...
    def forward_with_w(
            ...
            if len(styles) < 2:
                ...
                # NOTE: if styles is (N, dim), then style is repeated to all layer, else in w+ space.
                if styles[0].ndim < 3:
            ...
            latent.requires_grad_(True) # (N, N_layer, w_dim)
            ...
            if return_feats:
                return image, latent, feat_list
            if return_latents:
                return image, latent, inject_index
